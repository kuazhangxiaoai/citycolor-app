...
def plot_predict_samples(image, masks, nc, colors, fname, mname, one_hot=True, overlap=False, only_plot=None):
    """
    ...
    if only_plot is not None:
        if one_hot:
            ...
            r, g, b = colors[only_plot]
            mask_bgr[mask[:, :, only_plot] > 114, :] = np.array([b, g, r]).astype(np.uint8)
        else:
            r, g, b = colors[only_plot]
            mask_bgr[masks == only_plot, :] = np.array([b, g, r]).astype(np.uint8)
    else:
        if one_hot:
            mask = masks.copy().transpose(1, 2, 0)
            for j in range(nc):
